            ...
            try {
                ...
                if (!_.isEmpty(userAccount)) {
                    ...
                    if (checkPassword) {
                        ...
                        res.status(200).json({ message: yield notification('success'), accessToken, refreshToken });
                    }
                    else {
    ...
    refreshToken(req, res) {
        return __awaiter(this, void 0, void 0, function* () {
            try {
                ...
                const decoded = jwt.verify(refreshToken, process.env.SECRET_OR_KEY_REFRESH);
                const accessToken = jwt.sign({ id: (decoded.id), role: decoded.role }, process.env.SECRET_OR_KEY, { expiresIn: '15m' });
                res.status(200).json({ message: yield notification('success'), accessToken });
            }
            catch (error) {
