...
class AccountController {
    public async loginAccount (req: Request, res: Response) {
        ...
        try {
            ...
            if (!_.isEmpty(userAccount)) {
                ...
                if (checkPassword) {
                    const accessToken = jwt.sign({ id: userAccount.id, role: userAccount.role }, process.env.SECRET_OR_KEY, { expiresIn: '15m' });
                    const refreshToken = jwt.sign({ id: userAccount.id, role: userAccount.role }, process.env.SECRET_OR_KEY_REFRESH, { expiresIn: '7d' });
                    res.status(200).json({ message: await notification('success'), accessToken, refreshToken });
                } else {
    ...
    public async refreshToken (req: Request, res: Response) {
        try {
            ...
            const decoded = jwt.verify(refreshToken, process.env.SECRET_OR_KEY_REFRESH) as JwtPayload;
            const accessToken = jwt.sign({ id: (decoded.id), role: decoded.role }, process.env.SECRET_OR_KEY, { expiresIn: '15m' });
            res.status(200).json({ message: await notification('success'), accessToken });
        } catch (error) {
